...
def compute_entropy(data):
    ...
    if s == 0:
        return 0
    for x in data:
        total_entropy += - x / s * math.log2(x / s)
...
def compute_info_gain(data, att_idx):
    info_gain = 0.0
    ########## Please Fill Missing Lines Here ##########
    labels = [data.label[i] for i in range(data.num_instances)]
    set_label = set(data.label)
    total = Counter(labels)
    info_gain = compute_entropy([total[x] for x in set_label])
    for a in data.attr_set:
        sub = Counter([data.label[i] for i in range(data.num_instances) if data.attrs[i][att_idx] == a])
        info_gain -= compute_entropy([sub[x] for x in set_label])
    return info_gain
...
def comput_gain_ratio(data, att_idx):
    ...
    splits = Counter(data.attrs[att_idx])
    for a in data.attr_set:
        splitInfo += - splits[a] / data.num_instances * math.log2(splits[a] / data.num_instances)
    return gain_ratio
    ...
    def make_tree(self):
        if self.instances.num_instances == 0:
            ...
            if np.abs(gains[self.m_attr_idx]) < 1e-9:
                ...
                ########## Please Fill Missing Lines Here ##########
                outcomes = Counter(self.instances.label)
                self.m_class = outcomes.most_common(1)[0][0]
